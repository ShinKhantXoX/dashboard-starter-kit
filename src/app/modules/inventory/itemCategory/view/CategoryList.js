...
import { ItemCategorySearch } from "../entry/ItemCategorySearch";

export const CategoryList = () => {
    useDocumentTitle("Item Categories");
    
    const [loading, setLoading] = useState(false);
    const [records, setRecord] = useState([]);
    const [total, setTotal] = useState(0);
    const [params, setParams] = useState(itemCategoryParamsInit);
    ...
    const loadingData = useCallback(async () => {
        setLoading(true);
        const response = await getReqeust("item/category", params);
        ...
        <Grid gutter={0}>
            <Grid.Col span={12} my={10}>
                ...
                >
                    ...
                    <Group> 
                        <NavButton label="Create" disabled={loading} click={() => navigate("/inventory/category/new")} />
            ...
            <Grid.Col my={10}>
                <Card p={20} className="card-border">
                    <Card.Section my={10}>
                        <Text sx={{ fontSize: 20, fontWeight: 500}}> Item Category List </Text>
