...
import { countryParamsInit } from "../useCountryParams";
import { minHeight, paginationSize, recordsPerPageOptions } from "../../../config/datatable";
import { CountrySearch } from "../entry/CountrySearch";

export const CountryList = () => {
    useDocumentTitle("Country List");

    const [loading, setLoading] = useState(false);
    const [records, setRecord] = useState([]);
    const [total, setTotal] = useState(0);
    const [params, setParams] = useState(countryParamsInit);
    ...
    const columns = [
        ...
        { accessor: "country_photo", title: 'Country', sortable: true, render: ({country_photo}) => {
            return (
                <>
                    {
                        country_photo ? (
                            <Image
                            src={country_photo ? country_photo : null} 
                            width={50}
                            height={50}
                            mx={'auto'}
                            withPlaceholder
    ...
    const loadingData = useCallback(async () => {
        setLoading(true);
        const response = await getReqeust("country/list", params);
        ...
        <Grid gutter={0}>
            <Grid.Col span={12} my={10}>
                ...
                >
                    ...
                    <Group> 
                        <NavButton label="Create" disabled={loading} click={() => navigate("/country/new")} />
            ...
            <Grid.Col my={10}>
                <Card p={20} className="card-border">
                    <Card.Section my={10}>
                        <Text sx={{ fontSize: 20, fontWeight: 500}}> Country List </Text>
